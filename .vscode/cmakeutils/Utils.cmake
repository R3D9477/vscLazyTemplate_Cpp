...
set(IS_IN_VCPKGJSON FALSE)
file(READ "${CMAKE_SOURCE_DIR}/vcpkg.json" VCPKG_PACKAGES)
function(IS_VCPKGJSON_CONTAINS EXT_PKG_NAME IS_IN_VCPKGJSON)
    set(IS_IN_VCPKGJSON FALSE)
    if("${VCPKG_PACKAGES}" MATCHES "${EXT_PKG_NAME}")
        set(IS_IN_VCPKGJSON TRUE)
    endif()
    return (PROPAGATE IS_IN_VCPKGJSON)
endfunction()
    ...
    foreach(SRC_MASK_TEST ${SOURCE_MASK_TEST})
        ...
        foreach(TEST_SRC ${TEST_SRCs})
            ...
            if(DEFINED ENV{VCPKG_ROOT})
                IS_VCPKGJSON_CONTAINS("gtest" IS_IN_VCPKGJSON)
                if(${IS_IN_VCPKGJSON})
                ...
                IS_VCPKGJSON_CONTAINS("boost-test" IS_IN_VCPKGJSON)
                if(${IS_IN_VCPKGJSON})
...
function(target_add_boost TARGET_NAME)
    IS_CONANFILE_CONTAINS("boost" IS_IN_CONANFILE)
    IS_VCPKGJSON_CONTAINS("\"boost\"" IS_IN_VCPKGJSON)
    if(${IS_IN_CONANFILE} OR ${IS_IN_VCPKGJSON})
        ...
        if(Boost_FOUND)
            ...
            if(DEFINED ENV{VCPKG_ROOT})
                ...
                target_link_libraries(${TARGET_NAME} PUBLIC boost_thread)
            endif()
            if(WIN32)
            ...
            message("")
            message("--> Boost " ${Boost_VERSION} " has been added to " ${TARGET_NAME})
            message("")
        endif()
    endif()
endfunction()
